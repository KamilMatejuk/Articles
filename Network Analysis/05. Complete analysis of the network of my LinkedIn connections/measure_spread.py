...
from simulator import State, SimluatorEmpidemicSEIR
...
def graph_spread_from_each_node(graph: nx.Graph):
    nodes, reached_ratios, iterations = [], [], []
    for n in tqdm.tqdm(graph.nodes):
        nodes.append(n)
        irs = []
        its = []
        for _ in range(5):
            ir_i, it_i = get_spread(graph, n)
            irs.append(ir_i)
            its.append(it_i)
    ...
    show_spreads(nodes, reached_ratios, iterations, 'images/reach_all_seir_ir_0.15_1_3.png')
    print('avg reached_ratios', np.mean(reached_ratios))
    print('avg iterations', np.mean(iterations))


def graph_spread_with_each_node_removed(graph: nx.Graph):
    nodes, reached_ratios, iterations = [], [], []
    for n in tqdm.tqdm([None] + list(graph.nodes)):
